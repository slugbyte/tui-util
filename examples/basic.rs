...
use tui_util::key::Key;
use tui_util::window::Window;

fn main() {
    let mut win = Window::new().unwrap();
    // print line works like normal before raw mode is enabled
    ...
    win.write("type a 4 letter word and hit enter to continue\n> ")
        .unwrap();
    win.event_loop(&mut |event, _window| match event {
    ...
    // after rawmode is enabled its better to manually move the cursor and then
    // write strings.
    ...
    win.write("\n\x1b[1E").unwrap(); // put cursor on next line
                                     // when the window is dropped from scope it will automatically restore the termios
